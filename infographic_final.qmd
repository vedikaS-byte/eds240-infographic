...
I ended up changing my first graph (violin-boxplot) to a correlogram (heatmap). I agree that violin and boxplots may be difficult to interpret, so I opted for a heatmap as a more visually intuitive way to associate numeric metrics (ie.

**4. Address which (if any) of the [ten design elements](https://eds-240-data-viz.github.io/course-materials/assignments/final-project.html#partI) you *do not* plan to implement (remember, not all ten will necessarily apply to your project, and that’s okay!).**
...
spotify_clean %>%
  ...
  labs(
    ...
    subtitle = "Energy and loudness are strongly correlated, while instrumentalness and speechiness weakly relate to other features.",
...
energy_dance_df <- spotify_clean %>%
 ...
 energy_dance_df %>%
  ...
  scale_color_manual(values = c("low" = "#e6e6e6", "high" = "#1DB954"),
 ...
 theme_minimal() +
  ...
  ) + annotate("text", label= "Some of the least popular songs fall into an additional cluster within the \nambient and wellbeing genres.", x = .6, y =.2, color = "#1DB954", size = 6, 
               fontface = "bold")
   # annotate("text", label= "Density contours indicate where individual songs concentrate in energy-danceability space.", x = .15, y =.8, color = "#1DB954", size = 6,
```
...
genre_pct %>%
  ...
  theme(
   ...
   axis.title.x = element_text(color = "white", size = 20, face = "bold"),
    axis.title.y = element_text(color = "white", size = 20, face = "bold")
  ) + annotate("text", label= "The least popular songs fall into a separate cluster here within the ambient and wellbeing genres.", x = .5, y =.2)
...
![](images/clipboard-1183148375.png)
